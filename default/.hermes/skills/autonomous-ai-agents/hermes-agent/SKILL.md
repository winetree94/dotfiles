...
#### Web search backend quick setup

Hermes web search uses the `web` config section plus provider-specific env vars. For a self-hosted SearXNG instance:
...
Verify without relying on the current agent session's already-loaded env:
...
print(web_search_tool('Hermes Agent NousResearch', limit=2)[:1000])
PY
```

Notes: SearXNG is search-only (`web.search_backend`); keep `web.extract_backend` on Firecrawl/Tavily/Exa/etc. if page extraction/crawling is needed. Config/env changes require a fresh session or gateway `/restart` to be available to the running agent process.
...
Hermes built-in curated memory is stored as two editable Markdown files:

```bash
~/.hermes/memories/MEMORY.md   # environment facts, project conventions, tool quirks
~/.hermes/memories/USER.md     # user profile, preferences, communication style
```

To inspect or edit directly:

```bash
less ~/.hermes/memories/MEMORY.md
less ~/.hermes/memories/USER.md
nano ~/.hermes/memories/MEMORY.md
nano ~/.hermes/memories/USER.md
```

Agents should still prefer the `memory` tool (`add`, `replace`, `remove`) for normal edits because it enforces limits and safety checks. Memory is snapshotted into the system prompt at session start, so file/tool edits persist immediately but are not reflected in the running prompt until a new session or gateway `/restart`/`/reset`.
