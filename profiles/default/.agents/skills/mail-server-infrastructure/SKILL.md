...
description: 개인 메일 서버(mail.winetree94.com)·웹메일(webmail.winetree94.com)과 관련 외부 인프라를 관리한다. 설정·배포, DNS·TLS·송수신 진단, 백업·복구 작업에 사용한다.
---
# 메일 서버 인프라 관리 가이드

작업 전에 인벤토리에서 대상, 접근 방법, 관리 정본을 확인한다.

| 대상               | 용도                           | 접근 방법                         | 관리 위치                                        | 비고  |
| ---------------- | ---------------------------- | ----------------------------- | -------------------------------------------- | --- |
| mail-server 클러스터 | 메일·웹메일 서비스                   | Kubernetes 컨텍스트 `mail-server` | `~/Workspaces/tinyrack/mail-server` (GitOps) | -   |
| 클러스터 호스트         | OS·K3S·네트워크 설정               | SSH `mail-server`             | 같은 저장소의 `ansible/`                           | -   |
| 외부 리소스           | Hetzner·Cloudflare·오브젝트 스토리지 | 공급자 CLI·rclone                | [외부 인프라 문서](references/external.md)          | -   |
대표 메일 호스트명은 `mail.winetree94.com`이다.
...
| DNS·TLS·메일 송수신·웹메일 진단             | 위 저장소의 지침·README와 해당 설정 |
...
Tinyrack 클라우드는 [tinyrack-infrastructure](../tinyrack-infrastructure/SKILL.md)가 담당하는 별도 환경이다.
